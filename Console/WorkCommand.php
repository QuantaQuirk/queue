...
class WorkCommand extends Command
{
     ...
     *
     * @param  \QuantaQuirk\Queue\Worker  $worker
    ...
    public function handle()
    {
        ...
        $connection = $this->argument('connection')
                        ?: $this->quantaquirk['config']['queue.default'];
     ...
     *
     * @return \QuantaQuirk\Queue\WorkerOptions
     */
    ...
    {
        $this->quantaquirk['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->quantaquirk['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->quantaquirk['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->quantaquirk['events']->listen(JobFailed::class, function ($event) {
     ...
     *
     * @return \QuantaQuirk\Support\Carbon
     */
    protected function now()
    {
        $queueTimezone = $this->quantaquirk['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->quantaquirk['config']->get('app.timezone')) {
     ...
     *
     * @param  \QuantaQuirk\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->quantaquirk['queue.failer']->log(
    ...
    {
        return $this->option('queue') ?: $this->quantaquirk['config']->get(
    ...
    {
        return $this->option('force') ? false : $this->quantaquirk->isDownForMaintenance();
    }
}
