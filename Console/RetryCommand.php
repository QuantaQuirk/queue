...
use QuantaQuirk\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->quantaquirk['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->quantaquirk['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->quantaquirk['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->quantaquirk['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->quantaquirk['queue.failer']->all())
                        ->where('queue', $queue)
                        ->pluck('id')
                        ->toArray();
    ...
    {
        $this->quantaquirk['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->quantaquirk->bound(Encrypter::class)) {
            $instance = unserialize($this->quantaquirk->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
